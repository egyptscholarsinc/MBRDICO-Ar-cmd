 ...
 * Version : 0.99
 * Time-stamp: <1999-06-23 12:25:53 pagel> 
 * 
...
SpeakFrench::SpeakFrench(istream& zscore, istream& exception, istream& decision, const char* dba): Speak(zscore,exception,decision,dba)
{
  strcpy(mbrola_database,dba);
}

// Spread accents and return basic speed (the shortest the word, the slowest the pace)
// Thonification is to tones, what mayonnaise is to tuna fish
double 
SpeakFrench::thonification(LPhonemePlus& lpp) const
{
...
void 
SpeakFrench::prosodification(LPhonemePlus& lpp, double init_f0, double ambitus, double speed) const
{
		...
		for (size_t i=0; i < proso.size() ; i++)
		  {
			 ...
			 switch (diacritic)
				...
				case '=':
				  pp.append_pitch( PitchPoint(0.0,init_f0) ); // init pitch
				  break;
